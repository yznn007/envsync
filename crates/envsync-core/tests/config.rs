...
fn replace_backend(text: &str, replacement: &str) -> String {
    text.replace(&format!("\"{}\"", backend_yaml()), replacement)
}
...
fn v2_git_backend_is_parsed_and_rejects_credentials_in_url() {
    let local_backend = format!("backend:\n  kind: local\n  path: \"{}\"", backend_yaml());
    let git_backend = "backend:\n  kind: git\n  remote_url: \"ssh://git@example.invalid/envsync.git\"\n  auth:\n    kind: ssh-agent";
    let text = yaml_with("")
        .replace(&local_backend, git_backend)
        .replace("version: 1", "version: 2");
...
fn rule08_structured_merge_survives_yaml_round_trip() {
    ...
    let again = WorkspaceConfig::parse_yaml(&text, &absolute_path("/tmp")).expect("往返可解析");
...
fn rule14_relative_paths_resolve_against_base_dir() {
    let text = replace_home(
        &replace_backend(&yaml_with("state_dir: .envsync\n"), "\"backend\""),
        "\"home\"",
    );
